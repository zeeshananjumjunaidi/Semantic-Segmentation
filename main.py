...
def load_vgg(sess, vgg_path):
    ...
    layer_3_out = tf.get_default_graph().get_tensor_by_name(vgg_layer3_out_tensor_name)
    layer_4_out = tf.get_default_graph().get_tensor_by_name(vgg_layer4_out_tensor_name)
    layer_7_out = tf.get_default_graph().get_tensor_by_name(vgg_layer7_out_tensor_name)
    return image_input, keep_prob, layer_3_out, layer_4_out, layer_7_out
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # skip connection (element-wse addition)
...
def run():
    ...
    with tf.Session() as sess:
        ...
        batch_size=5
        # TF placeholders
        correct_label = tf.placeholder(tf.int32,[None,None,None,num_classes],name='corrected_label')
